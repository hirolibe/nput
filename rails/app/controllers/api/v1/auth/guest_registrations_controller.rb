  ...
  private
    ...
    def create_guest_account(token)
      ActiveRecord::Base.transaction do
        user = User.create!(uid: token["sub"], email: generate_random_email, name: generate_random_name, terms_version: params[:terms_version],
                            privacy_version: params[:privacy_version], agreed_at: Time.current, guest: true)
